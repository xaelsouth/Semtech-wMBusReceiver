 ...
 */
extern thread_t *sx1272_control_thread;
 ...
 */
void _SX1272OnDio0Irq( void *context );
void _SX1272OnDio1Irq( void *context );
void _SX1272OnDio2Irq( void *context );
void _SX1272OnDio3Irq( void *context );
void _SX1272OnDio4Irq( void *context );
void _SX1272OnDio5Irq( void *context );
void _SX1272OnTimeoutIrq( void *context );
 ...
 */
static const RadioRegisters_t RadioRegsInit[] = SX1272_RADIO_INIT_REGISTERS_VALUE;
...
void SX1272HwInit( SPIDriver *spip )
{
    /*
    ...
    SX1272.Spi = spip;
    SX1272.Reset = LINE_SX1272_RESET;
#if (SX1272_DIO0_IN_USE == 1)
    SX1272.DIO0 = LINE_SX1272_DIO0;
#endif
    SX1272.DIO1 = LINE_SX1272_DIO1;
    SX1272.DIO2 = LINE_SX1272_DIO2;
#if (SX1272_DIO3_IN_USE == 1)
    SX1272.DIO3 = LINE_SX1272_DIO3;
#endif
#if (SX1272_DIO4_IN_USE == 1)
    SX1272.DIO4 = LINE_SX1272_DIO4;
#endif
#if (SX1272_DIO5_IN_USE == 1)
    SX1272.DIO5 = LINE_SX1272_DIO5;
#endif
}
...
void SX1272Init( RadioEvents_t *events )
{
    ...
    TimerInit( &TxTimeoutTimer, _SX1272OnTimeoutIrq );
    TimerSetContext( &TxTimeoutTimer, (void *)EVT_RF_TX_TIMEOUT );

    TimerInit( &RxTimeoutTimer, _SX1272OnTimeoutIrq );
    TimerSetContext( &RxTimeoutTimer, (void *)EVT_RF_RX_TIMEOUT );
    
    TimerInit( &RxTimeoutSyncWord, _SX1272OnTimeoutIrq );
...
void SX1272SetDeviation( uint16_t fdev )
{
...
void SX1272WriteBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    //NSS = 0;
    spiSelect(SX1272.Spi);

    addr |= 0x80;

    spiExchange(SX1272.Spi, 1, &addr, &dummy);
    for( i = 0; i < size; i++ )
    {
      spiExchange(SX1272.Spi, 1, &buffer[i], &dummy);
    }

    //NSS = 1;
    spiUnselect(SX1272.Spi);
}
...
void SX1272ReadBuffer( uint16_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    for( i = 0; i < size; i++ )
    {
      spiExchange(SX1272.Spi, 1, &dummy, &buffer[i] );
    }
...
void _SX1272OnTimeoutIrq( void *context )
{
  chEvtSignal(sx1272_control_thread, (eventmask_t)context);
}
...
{
  (void)context;
  chEvtSignalI(sx1272_control_thread, EVT_RF_DIO0);
}

void _SX1272OnDio1Irq( void *context )
{
  (void)context;
  chEvtSignalI(sx1272_control_thread, EVT_RF_DIO1);
}
...
void _SX1272OnDio2Irq( void *context )
{
  ...
  chEvtSignalI(sx1272_control_thread, EVT_RF_DIO2);
}

void _SX1272OnDio3Irq( void *context )
{
  (void)context;
  chEvtSignalI(sx1272_control_thread, EVT_RF_DIO3);
}

void _SX1272OnDio4Irq( void *context )
{
  (void)context;
  chEvtSignalI(sx1272_control_thread, EVT_RF_DIO4);
}

void _SX1272OnDio5Irq( void *context )
{
  (void)context;
  chEvtSignalI(sx1272_control_thread, EVT_RF_DIO5);
}
    ...
    switch( SX1272.Settings.State )
    {
        case RF_RX_RUNNING:
            ...
            case MODEM_FSK:
                ...
                if( ( SX1272.Settings.FskPacketHandler.Size == 0 ) && ( SX1272.Settings.FskPacketHandler.NbBytes == 0 ) )
                {
                    SX1272ReadFifo( RxTxBuffer, SX1272.Settings.FskPacketHandler.FifoThresh - 1);
                    SX1272.Settings.FskPacketHandler.NbBytes = SX1272.Settings.FskPacketHandler.FifoThresh - 1;

                    if( SX1272.Settings.Fsk.FixLen == false )
                    {
                         SX1272.Settings.FskPacketHandler.Size = RxTxBuffer[0];
                    }
                    else
                    {
                        SX1272.Settings.FskPacketHandler.Size = SX1272Read( REG_PAYLOADLENGTH );
                        ...
                        if (SX1272.Settings.FskPacketHandler.Size == 0)
                        {
                          ...
                          { /* Mode C1 datagram. */
                            ...
                            SX1272.Settings.FskPacketHandler.Size = 2 + FULL_DTG_LENGTH_FROM_L_FIELD[L];
                          }
                          else
                          { /* Mode T1 datagram. */
                            ...
                            if ( err_3outof6 )
                            {
                              ...
                              else
                              {
                                ...
                                SX1272.Settings.FskPacketHandler.Size = t1_3out6_packet_length(FULL_DTG_LENGTH_FROM_L_FIELD[L]);
                              }
                            }
                            else
                            {
                ...
                if (SX1272.Settings.FskPacketHandler.NbBytes >= SX1272.Settings.FskPacketHandler.Size)
                {
                  TimerStop( &RxTimeoutTimer );

                  if( ( RadioEvents != NULL ) && ( RadioEvents->RxDone != NULL ) )
                  {
                    RadioEvents->RxDone( RxTxBuffer, SX1272.Settings.FskPacketHandler.Size,
                                         SX1272.Settings.FskPacketHandler.RssiValue, 0 );
                  }
